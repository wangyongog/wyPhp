...
class DB{
    ...
    static function _connect(array $options=[]) {
        $db = isset($options['prefix']) ? $options['prefix'] : CF::get('DEFAULT_DB');
        $master = isset($options['slave']) ? $options['slave'] : 'master';
        if(!isset(self::$_db[$db][$master])){
            $dbConfig = CF::get($db);
            empty($dbConfig) and CF::get('db');
    ...
    static function checkSlave($params, $master=true){
        ...
        if(CF::get('RW_SEPARATE') === true && $master === false){
            //指定了从库，以指定为准
 ...
 * @return mixed
 */
    public static function fetch_all($table, $fields='*', $condition=null, $order='', $limit='', $page=0 , $group='', $having = '', $params=[]){
