...
class task extends baseController {
    ...
    public function actionTask(){
        if(IS_AJAX){
            $data = $this->_task_list('task');
            $html_row = '';
            if($data){
                foreach ($data as $val){
                    $hands = !$val['status'] && !$val['isclose'] ? '<a href="javascript:;" class="tipClose" _url="'.U('task/del',['tid'=>$val['tid'],'token'=>creatToken($val['tid'])]).'">关闭</a>' : '';
                    $html_row .= '<tr>
                ...
                $html_row = '<td colspan="10" align="center" style="line-height:45px"><i class="fa fa-fw fa-exclamation-triangle"></i>没有查询到符合条件的记录</td>';
            }
    ...
    public function actionDianzan(){
        if(IS_AJAX){
            ...
            if($data){
                ...
                $html_row = '<td colspan="9" align="center" style="line-height:45px"><i class="fa fa-fw fa-exclamation-triangle"></i>没有查询到符合条件的记录</td>';
            }
            $this->tbody_html = $html_row;
            $this->dynamicOutPrint();
        }
    ...
    public function actionTaocan(){
        if(IS_AJAX){
            ...
            if($data){
                ...
                $html_row = '<td colspan="12" align="center" style="line-height:45px"><i class="fa fa-fw fa-exclamation-triangle"></i>没有查询到符合条件的记录</td>';
            }
            $this->tbody_html = $html_row;
            $this->dynamicOutPrint();
        }
        $task_config = F('task');
        $task_config = $task_config['SHOW_LIST']['taocan'];
        $this->assign('show_list', $task_config);
        $this->render();
    }
    public function actionTaocanadd(){
        ...
        $price = BalanceModel::get_member_price($this->uid, 'taocan', $this->user['grade']);
        if($price<=0 ){
        ...
        if(IS_POST) {
            ...
            if($create['count']>UPNUM){
                $this->error('任务上限为'.UPNUM.'!');
            }
    ...
    public function actionFansadd(){
        $price = BalanceModel::get_member_price($this->uid, 'fans', $this->user['grade']);
        if($price<=0 ){
        ...
        if(IS_POST) {
            ...
            if($create['count']<500 || $create['count']>UPNUM){
                $this->error('加粉数量必须500-'.UPNUM.'!');
            }
            $create['from'] = 1;
    ...
    public function actionTaskadd(){
        ...
        $price = BalanceModel::get_member_price($this->uid, 'task', $this->user['grade']);
        $price1 = BalanceModel::get_member_price($this->uid, 'gaosu', $this->user['grade']);
        if(!$price || !$price1){
    ...
    public function actionDianzanadd(){
        $price = BalanceModel::get_member_price($this->uid, 'dianzan', $this->user['grade']);
        if(!$price){
        ...
        if(IS_AJAX){
            ...
            $create['count'] = intval(I('count'));
            $create['thumb'] = 0;
            if($create['count']<100 || $create['count']>UPNUM){
                $this->error ('任务数量必须100-'.UPNUM.'!');
            }
            $get_data = TaskModel::get_curl_title($create['url']);
            $create['title'] = $get_data['title'];
            $create['weixin_no'] = isset($get_data['weixin_no']) ? $get_data['weixin_no'] : '';
